...
static Lisp table_get_string(Lisp l, const char* string, unsigned int hash)
{
    ...
    while (!lisp_is_null(it))
    {
        ...
        if (_stricmp(lisp_symbol(symbol), string) == 0)
        {
            return pair;
        }
...
static Lisp lisp_stack_pop(LispContext ctx)
{
    ...
    if (LISP_DEBUG)
    {
        if ((long long)ctx.impl->stack_ptr < 0)
        {
    ...
    switch (l.type)
    {
        ...
        case LISP_TABLE:
        {
            ...
            if (!(table->block.gc_flags & GC_MOVED))
            {
                ...
                {
                    fprintf(stdout, "resizing table %i -> %i\n", table->capacity, new_capacity);
                }
...
static Lisp sch_display(Lisp args, LispError* e, LispContext ctx)
{
    ...
    {
        fprintf(stderr, "%s", lisp_string(l));
    }
    else
    {
        lisp_print(l);
...
static Lisp sch_to_string(Lisp args, LispError* e, LispContext ctx)
{
    ...
    {
        case LISP_REAL:
            _snprintf(scratch, SCRATCH_MAX, "%f", lisp_real(val));
            return lisp_make_string(scratch, ctx);
        case LISP_INT:
            _snprintf(scratch, SCRATCH_MAX, "%i", lisp_int(val));
            return lisp_make_string(scratch, ctx);
        case LISP_SYMBOL:
